...
//! then SIGKILLs — so on every rolling update the drain was skipped entirely and
//! whatever was in flight died with the process. D23 makes the blast radius
//! larger rather than smaller: each caller holds ONE long-lived HTTP/2
//! connection, so what is lost is everything that connection was carrying.
//!
...
async fn a_sigterm_drains_the_server_instead_of_killing_the_process() {
    ...
    // router that needs no `task-db` channel keeps the rig honest about that.
